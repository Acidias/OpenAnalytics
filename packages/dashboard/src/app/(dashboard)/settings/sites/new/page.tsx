...
export default function NewSitePage() {
  ...
  const siteId = "site_new_" + domain.replace(/\./g, "_");
  ...
  return (
    <div className="space-y-8 max-w-xl mx-auto">
      <div className="text-center">
        ...
        <p className="text-muted-foreground mt-1">Start tracking analytics in minutes</p>
      </div>
      ...
      <div className="flex items-center justify-center gap-2">
        {[1, 2, 3].map((s) => (
          <div key={s} className="flex items-center gap-2">
            ...
            {s < 3 && <div className={`w-12 h-px ${step > s ? "bg-primary" : "bg-muted"}`} />}
          </div>
        ))}
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="name" placeholder="My Website" value={siteName} onChange={(e) => setSiteName(e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="domain">Domain</Label>
              <Input id="domain" placeholder="example.com" value={domain} onChange={(e) => setDomain(e.target.value)} />
            </div>
            <Button className="w-full" onClick={() => setStep(2)} disabled={!siteName || !domain}>Continue</Button>
          </CardContent>
        </Card>
      )}
      ...
      {step === 2 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Add this snippet to your website&apos;s &lt;head&gt; tag</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="p-4 rounded-md bg-muted font-mono text-sm break-all">{scriptTag}</div>
            <Button variant="outline" className="w-full" onClick={handleCopy}>
              {copied ? <><Check className="mr-2 h-4 w-4" /> Copied!</> : <><Copy className="mr-2 h-4 w-4" /> Copy Script</>}
            </Button>
            <Button className="w-full" onClick={() => setStep(3)}>I&apos;ve Added the Script</Button>
          </CardContent>
        </Card>
      )}
      ...
      {step === 3 && (
        <Card>
          <CardHeader className="text-center">
            ...
            <CardDescription>{siteName} ({domain}) is now being tracked</CardDescription>
          </CardHeader>
          <CardContent>
